...
public class Statistics
{
    ...
    {        
        if (leaderboardList.Count>1)Utilities.SortPoints(leaderboardList);
        Console.Clear();
        ...
        for (int i = 0; i < 10 && i< leaderboardList.Count; i++)
        {
            if(leaderboardList[i] != null)
            {
                ...
                Console.WriteLine(String.Format("{0,-20}{1,-30}{2,-14}{3,-13}{4,-15}{5,-14}{6,-0}", leaderboardList[i].Name, leaderboardList[i].Team.Name, leaderboardList[i].GamesPlayed, leaderboardList[i].TotalGoal, leaderboardList[i].TotalAssist, leaderboardList[i].TotalPoint, leaderboardList[i].Plusminus));
            }                
        }
        Console.WriteLine("\n");
        Utilities.KeyPress();       
    ...
    internal static void Individual(Player p)
    {
        ...
        Console.WriteLine(String.Format("{0,-5}{1,-14}{2,-13}{3,-15}{4,-14}{5,-15}{6,-15}{7,-15}{8,-15}","", p.GamesPlayed, p.TotalGoal, p.TotalAssist, p.TotalPoint, p.Plusminus, p.TotalPenalty,p.TotalHit ,p.TotalBlock));
        Console.WriteLine("\n\n");
        Utilities.KeyPress();
    }
    ...
    internal static void LeaderboardCheck()
    {
        foreach (Team t in Team.list)
        {
            foreach (Player p in t.Roster)
            {
                if (p != null)
                {
                    if (!leaderboardList.Contains(p) && p.pointStat[Season.gameWeek] > 0) leaderboardList.Add(p);
                }
            }
        }
    }
